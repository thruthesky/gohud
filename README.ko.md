...
**버전 1.2.0.** 32개 묶음의 아이콘 1,000개 라이브러리(`GoIconLibrary`)와 이름 검색, 프리셋을 바꿔도
남는 추가 아이콘 세트, 그 아이콘으로 만든 버튼, 17개 언어의 아이콘 페이지, 누르는 칸 안의 내용을
배치하는 공통 규칙, 320dp 폰에서도 모든 위젯을 화면 안에 두는 화면 전체 배치 점검이 들어 있다 —
