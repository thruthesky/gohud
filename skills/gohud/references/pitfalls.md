...
| The spacing you gave a column comes out as 12 | Before 2026-09-23 `GoStyle.form()` — run by every `GoForm` on everything added — set every box's `separation` to `GAP` | It now keeps any spacing that is not `GAP`, and any height you set. To keep a box at exactly `GAP` against a later form pass, `box.set_meta(&"go_own_spacing", true)` |
| A count badge is a big square, a `+` or `…` is 1dp wide, inside a form only | `GoForm` turns wrapping on for every label left at `AUTOWRAP_OFF`; a wrapping label is 1dp wide and grows tall | `GoStyle.one_line(label)` — wrapping off **and** marked so the form leaves it |
| The whole page is cut off on the right on a phone | One widget's minimum width (a 12-page pager, 457 dp) is wider than the screen, and nothing scrolls sideways | A widget must fit the width it is given — `GoPagination` folds, `GoCodeInput` wraps its groups. Find the culprit with `GoStyle.audit_layout(root)` |
| A carousel slices its banner in half | Its pages were sized to the viewport, and a plain `Control` does not take its children's height | Fixed: it is as tall as its tallest page. `custom_minimum_size` is a floor, not the room |
| A `GoStyle.card()` has no padding and a 3 dp corner in a project with no theme of its own | The card read its face from the theme before the theme had settled on the new node, and got the engine's bare panel | Fixed (2026-09-23): the face is read from the theme itself |
